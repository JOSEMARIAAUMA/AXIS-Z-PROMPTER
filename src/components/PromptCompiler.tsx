...
import { CompiledPrompt, CategoryMap, PromptItem, LibrarySuggestion, AreaType } from '../types';
...
interface PromptCompilerProps {
  ...
  onUpdateComposition: (data: CompiledPrompt) => void;
...
export const PromptCompiler: React.FC<PromptCompilerProps> = ({ 
    compiler, activeCompositionId, onChange, onClear, onSaveToLibrary, onUpdateComposition, availableCategories, availableApps,
  ...
  const handleSaveSubmit = () => {
      ...
      onSaveToLibrary(saveTitle, selectedCategories, selectedApps);
  ...
  const config = AREA_CONFIG[currentArea].compiler;
                     ...
                     <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                        {Object.keys(availableCategories).map(cat => (
                            <button
                                ...
                                className={`text-left px-3 py-2 rounded-md text-xs border transition-all flex justify-between items-center ${
                                    selectedCategories.includes(cat)
                                    ? 'bg-accent-600 border-accent-600 text-white shadow-md'
                                    : 'bg-arch-950 border-arch-800 text-arch-400 hover:border-arch-600'
                                }`}
                            >
                                <span>{cat}</span>
                                {selectedCategories.includes(cat) && <Icons.Check size={12} />}
                            </button>
                        ))}
                     </div>
                 </div>
            ...
            >
             ...
             <button 
                ...
                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                    copied ? 'bg-green-600 text-white' : 'bg-accent-600 hover:bg-accent-500 text-white'
                }`}
            >
        ...
        <div className={`mb-3 rounded-lg border transition-all duration-200 ${collapsedSections['system'] ? 'bg-cyan-950/10 border-cyan-900/20' : 'bg-cyan-950/10 border-cyan-900/30'}`}>
             ...
             >
                ...
                <button 
                    ...
                    className={`flex items-center space-x-1 px-1.5 py-0.5 rounded text-[9px] font-medium transition-colors border ${
                        systemCopied 
