...
export const PromptList: React.FC<PromptListProps> = ({ 
  prompts, compositions, categories, selectedId, onSelect, onSelectComposition, onDeleteComposition, onAdd, onToggleFavorite, onAddToCompiler, onManageCategories, onOpenBackup, onManageApps, availableApps, filter, setFilter, currentArea 
}) => {
  ...
  // 2. Derive valid categories for the current area to keep UI clean
  const validCategories = useMemo(() => {
      const activeCats = new Set<string>();
      areaPrompts.forEach(p => activeCats.add(p.category));
      return Object.keys(categories).filter(cat => activeCats.has(cat) || Object.keys(categories).length === 0); 
  }, [categories, areaPrompts]);
  ...
  const clearFilters = () => {
    setFilter({ search: '', category: 'All', subcategory: 'All', app: 'All' });
  };
  ...
  const getIconForCategory = (cat: string) => {
    ...
    if (lower.includes('veg') || lower.includes('plant')) return <Icons.Vegetation size={16} />;
    if (lower.includes('pers') || lower.includes('gente') || lower.includes('peop')) return <Icons.People size={16} />;
    if (lower.includes('mat')) return <Icons.Materials size={16} />;
    if (lower.includes('luz') || lower.includes('light') || lower.includes('clima')) return <Icons.Lighting size={16} />;
    if (lower.includes('arq') || lower.includes('arch') || lower.includes('esti')) return <Icons.ArchStyle size={16} />;
    if (lower.includes('det')) return <Icons.Details size={16} />;
    if (lower.includes('cam') || lower.includes('vista')) return <Icons.Image size={16} />;
  ...
  // Determine which categories to show in the filter bar
  // If in 'LIBRARY' mode, show valid categories. If they are empty (new area), show all to allow selection for new items.
  const categoriesToShow = validCategories.length > 0 ? validCategories : Object.keys(categories);
    ...
    <div className="flex flex-col h-full bg-arch-900">
      ...
      <div className="flex p-2 bg-arch-950 border-b border-arch-800 space-x-2">
          <button 
            ...
            className={`flex-1 flex items-center justify-center space-x-2 py-2 rounded text-xs font-bold uppercase tracking-wide transition-colors ${
                repoMode === 'SNIPPETS' ? 'bg-arch-800 text-white shadow-sm' : 'text-arch-500 hover:text-arch-300'
            }`}
          >
          ...
          <button 
             ...
             className={`flex-1 flex items-center justify-center space-x-2 py-2 rounded text-xs font-bold uppercase tracking-wide transition-colors ${
                repoMode === 'COMPOSITIONS' ? 'bg-arch-800 text-white shadow-sm' : 'text-arch-500 hover:text-arch-300'
            }`}
          >
      ...
      <div className="flex border-b border-arch-800 bg-arch-950">
        <button 
          ...
          className={`flex-1 flex items-center justify-center space-x-2 py-3 text-xs font-medium border-b-2 transition-colors ${
            viewMode === 'LIBRARY' ? 'border-accent-500 text-white' : 'border-transparent text-arch-500 hover:text-arch-300'
          }`}
        >
        ...
        {repoMode === 'SNIPPETS' && (
            ...
            className={`flex-1 flex items-center justify-center space-x-2 py-3 text-xs font-medium border-b-2 transition-colors ${
                viewMode === 'FAVORITES' ? 'border-accent-500 text-white' : 'border-transparent text-arch-500 hover:text-arch-300'
            }`}
            >
            <Icons.Star size={14} />
            <span>Favoritos</span>
        ...
        <button 
          ...
          className={`flex-1 flex items-center justify-center space-x-2 py-3 text-xs font-medium border-b-2 transition-colors ${
            viewMode === 'HISTORY' ? 'border-accent-500 text-white' : 'border-transparent text-arch-500 hover:text-arch-300'
          }`}
        >
          <Icons.Clock size={14} />
          <span>Historial</span>
